...
// Tabs / filtering roles (ADMIN NAHI)
const ROLES = [
  "all",
  "driver",
  "Bus vendor",
...
const AccountantDashboard = () => {
  ...
  const fetchUsers = async (
    ...
    try {
      ...
      // yaha se admin users hata diye
      const filtered = rawUsers.filter((u) => u.role !== "admin");
      setUsers(filtered);
      setTotal(res.data.total ? filtered.length : filtered.length);
    } catch (err) {
  ...
  // total without admin for top card
  const totalWithoutAdmin =
    stats && stats.counts
      ? (stats.total || 0) - (stats.counts.admin || 0)
      : stats?.total || 0;
    ...
    >
      <div style={{ maxWidth: 1280, margin: "0 auto" }}>
        ...
        <div style={{ marginBottom: 24 }}>
          ...
          >
            View all data except admin users (read-only)
          </p>
        </div>
